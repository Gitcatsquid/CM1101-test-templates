...
def remove_punct(text):
    ...
    no_punct = ""
    for thing in text:
        if not (thing in string.punctuation):
            no_punct = no_punct + thing
    return no_punct
...
def print_menu(exits):
   ...
   #for direction in exits:
        #direction_destination = exit_leads_to(exits, direction)
        #print_menu_line(direction, direction_destination)
    for direction in exits:
...
def menu(exits):
    ...
    while True:
        ...
        user_input = input("> ")
        # Normalise the input
        chosen_exit = normalise_input(user_input)
